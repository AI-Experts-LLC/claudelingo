...
/**
 * When the old CLI's decks were read in, so it only ever happens once.
 *
 * Set whatever the outcome, including "there was nothing there" — otherwise a
 * machine that never had the old install pays a directory read every session
 * for ever.
 */
export const MIGRATED_KEY = 'claudelingo:migrated:v1'
