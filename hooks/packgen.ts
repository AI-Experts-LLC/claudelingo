 ...
 * in — `$.model.complete` in the band, a fake in the tests — which is what lets
 * the hardest code here be driven without a model at all.
 *
 * Every comment below records a real run that went wrong: a truncated reply
 * losing 693 words of French, boundaries overlapping until a 1000-word pack
 * held 600 distinct ones, and a model that, asked to fill a quota past the
 * point where it knows the frequency order, starts reciting the dictionary
 * alphabetically.
 */
