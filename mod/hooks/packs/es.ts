// Restated from src/packs/es.json by mod/scripts/restate.mjs.
// Do not edit: change src/packs/es.json and run `npm run mod:restate`.

import type { RawPack } from '../types'

export const ES: RawPack = {
  code: "es",
  name: "Español",
  englishName: "Spanish",
  words: [
  ["el","the (m.)","art","masculine"],
  ["los","the (m. pl.)","art"],
  ["de","of, from","prep"],
  ["que","that, which","conj"],
  ["y","and","conj"],
  ["la","the (f.)","art","feminine"],
  ["las","the (f. pl.)","art"],
  ["en","in, on","prep"],
  ["un","a (m.)","art"],
  ["una","a (f.)","art"],
  ["ser","to be (permanent)","verb","permanent qualities"],
  ["se","oneself","pron"],
  ["no","no, not","adv"],
  ["haber","to have (auxiliary)","verb","auxiliary"],
  ["por","for, by","prep"],
  ["con","with","prep"],
  ["su","his, her, their","adj"],
  ["para","for, to","prep"],
  ["como","like, as","conj"],
  ["estar","to be (state, place)","verb","states and places"],
  ["tener","to have (possess)","verb"],
  ["le","to him, to her","pron"],
  ["lo","it, the","pron"],
  ["todo","all, everything","adj"],
  ["pero","but","conj"],
  ["más","more","adv"],
  ["hacer","to do, to make","verb"],
  ["o","or","conj"],
  ["poder","to be able to","verb"],
  ["decir","to say","verb"],
  ["este","this","adj"],
  ["ir","to go","verb"],
  ["otro","other, another","adj"],
  ["ese","that (adjective)","adj"],
  ["si","if","conj"],
  ["me","me, myself","pron"],
  ["ya","already, now","adv"],
  ["ver","to see","verb"],
  ["porque","because","conj"],
  ["dar","to give","verb"],
  ["cuando","when","conj"],
  ["muy","very","adv"],
  ["sin","without","prep"],
  ["vez","time, occasion","noun","feminine"],
  ["mucho","much, a lot","adj"],
  ["saber","to know (facts)","verb","facts"],
  ["qué","what","pron"],
  ["sobre","on, about","prep"],
  ["mi","my","adj"],
  ["alguno","some, any","adj"],
  ["mismo","same, self","adj"],
  ["yo","I","pron"],
  ["también","also, too","adv"],
  ["hasta","until, up to","prep"],
  ["año","year","noun","masculine"],
  ["dos","two","num"],
  ["querer","to want, to love","verb"],
  ["entre","between, among","prep"],
  ["así","like this, so","adv"],
  ["primero","first","adj"],
  ["desde","from, since","prep"],
  ["grande","big, great","adj"],
  ["eso","that (pronoun)","pron"],
  ["ni","nor, neither","conj"],
  ["nos","us","pron"],
  ["llegar","to arrive","verb"],
  ["pasar","to pass, to happen","verb"],
  ["tiempo","time, weather","noun","masculine"],
  ["ella","she","pron"],
  ["sí","yes","adv"],
  ["día","day","noun","masculine"],
  ["uno","one","num"],
  ["bien","well","adv"],
  ["poco","little, few","adj"],
  ["deber","must, to owe","verb"],
  ["entonces","then","adv"],
  ["poner","to put","verb"],
  ["cosa","thing","noun","feminine"],
  ["tanto","so much","adj"],
  ["hombre","man","noun","masculine"],
  ["parecer","to seem","verb"],
  ["nuestro","our","adj"],
  ["tan","so, as","adv"],
  ["donde","where","adv"],
  ["ahora","now","adv"],
  ["parte","part","noun","feminine"],
  ["después","after, afterwards","adv"],
  ["vida","life","noun","feminine"],
  ["quedar","to stay, to remain","verb"],
  ["siempre","always","adv"],
  ["creer","to believe","verb"],
  ["hablar","to speak","verb"],
  ["llevar","to carry, to wear","verb"],
  ["dejar","to leave, to let","verb"],
  ["nada","nothing","pron"],
  ["cada","each, every","adj"],
  ["seguir","to follow, to continue","verb"],
  ["menos","less, fewer","adv"],
  ["nuevo","new","adj"],
  ["encontrar","to find","verb"],
  ["algo","something","pron"],
  ["sólo","only","adv"],
  ["casa","house","noun","feminine"],
  ["mujer","woman","noun","feminine"],
  ["trabajo","work, job","noun","masculine"],
  ["mano","hand","noun","feminine, irregular gender"],
  ["mundo","world","noun","masculine"],
  ["país","country","noun","masculine"],
  ["problema","problem","noun","masculine, irregular gender"],
  ["forma","form, shape, way","noun","feminine"],
  ["momento","moment","noun","masculine"],
  ["caso","case","noun","masculine"],
  ["punto","point","noun","masculine"],
  ["gente","people","noun","feminine"],
  ["lugar","place","noun","masculine"],
  ["agua","water","noun","feminine, takes el"],
  ["niño","child, boy","noun","masculine"],
  ["noche","night","noun","feminine"],
  ["ojo","eye","noun","masculine"],
  ["puerta","door","noun","feminine"],
  ["ciudad","city","noun","feminine"],
  ["nombre","name","noun","masculine"],
  ["padre","father","noun","masculine"],
  ["madre","mother","noun","feminine"],
  ["hijo","son, child","noun","masculine"],
  ["amigo","friend","noun","masculine"],
  ["libro","book","noun","masculine"],
  ["palabra","word","noun","feminine"],
  ["cabeza","head","noun","feminine"],
  ["dinero","money","noun","masculine"],
  ["historia","story, history","noun","feminine"],
  ["semana","week","noun","feminine"],
  ["mes","month","noun","masculine"],
  ["hora","hour, time","noun","feminine"],
  ["manera","way, manner","noun","feminine"],
  ["idea","idea","noun","feminine"],
  ["razón","reason","noun","feminine"],
  ["verdad","truth","noun","feminine"],
  ["fin","end","noun","masculine"],
  ["calle","street","noun","feminine"],
  ["pueblo","town, people","noun","masculine"],
  ["número","number","noun","masculine"],
  ["cuerpo","body","noun","masculine"],
  ["coche","car","noun","masculine"],
  ["comida","food, meal","noun","feminine"],
  ["escuela","school","noun","feminine"],
  ["papel","paper, role","noun","masculine"],
  ["fuerza","strength, force","noun","feminine"],
  ["guerra","war","noun","feminine"],
  ["cambio","change","noun","masculine"],
  ["campo","field, countryside","noun","masculine"],
  ["grupo","group","noun","masculine"],
  ["proyecto","project","noun","masculine"],
  ["empresa","company","noun","feminine"],
  ["gobierno","government","noun","masculine"],
  ["estado","state","noun","masculine"],
  ["sistema","system","noun","masculine, irregular gender"],
  ["ejemplo","example","noun","masculine"],
  ["clase","class, kind","noun","feminine"],
  ["cuenta","account, bill","noun","feminine"],
  ["orden","order","noun","masculine or feminine by sense"],
  ["salir","to leave, to go out","verb"],
  ["volver","to return","verb"],
  ["venir","to come","verb"],
  ["pensar","to think","verb"],
  ["mirar","to look at","verb"],
  ["conocer","to know (people, places)","verb","people and places"],
  ["vivir","to live","verb"],
  ["sentir","to feel","verb"],
  ["tomar","to take, to drink","verb"],
  ["trabajar","to work","verb"],
  ["escribir","to write","verb"],
  ["leer","to read","verb"],
  ["esperar","to wait, to hope","verb"],
  ["buscar","to look for","verb"],
  ["empezar","to begin","verb"],
  ["terminar","to finish","verb"],
  ["comer","to eat","verb"],
  ["beber","to drink","verb"],
  ["dormir","to sleep","verb"],
  ["abrir","to open","verb"],
  ["cerrar","to close","verb"],
  ["entrar","to enter","verb"],
  ["andar","to walk","verb"],
  ["correr","to run","verb"],
  ["jugar","to play","verb"],
  ["ganar","to win, to earn","verb"],
  ["perder","to lose","verb"],
  ["comprar","to buy","verb"],
  ["vender","to sell","verb"],
  ["pagar","to pay","verb"],
  ["cambiar","to change","verb"],
  ["ayudar","to help","verb"],
  ["usar","to use","verb"],
  ["contar","to count, to tell","verb"],
  ["mostrar","to show","verb"],
  ["preguntar","to ask","verb"],
  ["responder","to answer","verb"],
  ["recordar","to remember","verb"],
  ["olvidar","to forget","verb"],
  ["aprender","to learn","verb"],
  ["enseñar","to teach, to show","verb"],
  ["entender","to understand","verb"],
  ["explicar","to explain","verb"],
  ["necesitar","to need","verb"],
  ["gustar","to please, to like","verb","used as: me gusta"],
  ["intentar","to try","verb"],
  ["lograr","to achieve","verb"],
  ["crear","to create","verb"],
  ["construir","to build","verb"],
  ["escuchar","to listen","verb"],
  ["caer","to fall","verb"],
  ["subir","to go up, to upload","verb"],
  ["bajar","to go down, to download","verb"],
  ["traer","to bring","verb"],
  ["enviar","to send","verb"],
  ["recibir","to receive","verb"],
  ["romper","to break","verb"],
  ["morir","to die","verb"],
  ["nacer","to be born","verb"],
  ["crecer","to grow","verb"],
  ["bueno","good","adj"],
  ["malo","bad","adj"],
  ["mejor","better, best","adj"],
  ["peor","worse, worst","adj"],
  ["pequeño","small","adj"],
  ["largo","long","adj"],
  ["corto","short","adj"],
  ["alto","tall, high","adj"],
  ["bajo","low, short","adj"],
  ["joven","young","adj"],
  ["viejo","old","adj"],
  ["fácil","easy","adj"],
  ["difícil","difficult","adj"],
  ["importante","important","adj"],
  ["posible","possible","adj"],
  ["cierto","certain, true","adj"],
  ["claro","clear, of course","adj"],
  ["fuerte","strong","adj"],
  ["libre","free","adj"],
  ["lleno","full","adj"],
  ["frío","cold","adj"],
  ["caliente","hot","adj"],
  ["blanco","white","adj"],
  ["negro","black","adj"],
  ["rojo","red","adj"],
  ["verde","green","adj"],
  ["azul","blue","adj"],
  ["último","last","adj"],
  ["próximo","next","adj"],
  ["solo","alone, only","adj"],
  ["junto","together","adj"],
  ["propio","own","adj"],
  ["general","general","adj"],
  ["social","social","adj"],
  ["nacional","national","adj"],
  ["mayor","bigger, older","adj"],
  ["menor","smaller, younger","adj"],
  ["nunca","never","adv"],
  ["todavía","still, yet","adv"],
  ["quizá","maybe","adv"],
  ["además","besides, moreover","adv"],
  ["aunque","although","conj"],
  ["mientras","while","conj"],
  ["antes","before","adv"],
  ["luego","later, then","adv"],
  ["pronto","soon","adv"],
  ["tarde","late, afternoon","adv"],
  ["temprano","early","adv"],
  ["hoy","today","adv"],
  ["ayer","yesterday","adv"],
  ["mañana","tomorrow, morning","adv"],
  ["aquí","here","adv"],
  ["allí","there","adv"],
  ["cerca","near, close","adv"],
  ["lejos","far","adv"],
  ["dentro","inside","adv"],
  ["fuera","outside","adv"],
  ["arriba","up, above","adv"],
  ["abajo","down, below","adv"],
  ["detrás","behind","adv"],
  ["delante","in front","adv"],
  ["casi","almost","adv"],
  ["demasiado","too, too much","adv"],
  ["bastante","enough, quite","adv"],
  ["quién","who","pron"],
  ["cuál","which","pron"],
  ["cuánto","how much","pron"],
  ["cómo","how","adv"],
  ["dónde","where? (question)","adv"],
  ["por qué","why","adv"],
  ["nadie","nobody","pron"],
  ["todos","everyone, all","pron"],
  ["nosotros","we","pron"],
  ["ellos","they","pron"],
  ["usted","you (formal)","pron","formal"],
  ["tú","you (informal)","pron","informal"],
  ["tres","three","num"],
  ["cuatro","four","num"],
  ["cinco","five","num"],
  ["seis","six","num"],
  ["siete","seven","num"],
  ["ocho","eight","num"],
  ["nueve","nine","num"],
  ["diez","ten","num"],
  ["cien","hundred","num"],
  ["mil","thousand","num"],
  ["gracias","thank you","interj"],
  ["hola","hello","interj"],
  ["adiós","goodbye","interj"],
  ["favor","favour","noun","masculine, as in: por favor"],
  ["perdón","sorry, pardon","interj"],
  ],
}
