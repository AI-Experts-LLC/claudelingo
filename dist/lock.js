...
function releaseFor(file) {
    ...
    return {
        release() {
            ...
            if (current.state === "held" && current.lock.pid !== process.pid)
                return;
            try {
...
export function acquire(file) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            return { ok: false, reason: "unavailable", error: error };
        }
        ...
        if (current.lock.pid === process.pid)
            return { ok: true, lock: releaseFor(file) };
        if (alive(current.lock.pid))
            return { ok: false, reason: "held", pid: current.lock.pid };
...
export function holderPid(file) {
    const current = read(file);
