 ...
 */
/**
 * Zero-width code points: combining marks (Mn/Me) and format characters (Cf).
 *
 * Generated from the Unicode character database rather than hand-listed — a
 * hand-written table covered Latin accents and missed Devanagari, Arabic, Tibetan
 * and the rest, which matters because packs can be generated for any language.
 */
const ZERO_WIDTH = [
    [0x0300, 0x036f],
    [0x0483, 0x0489],
...
/**
 * Double-width code points: East Asian Wide and Fullwidth per Unicode TR11.
 *
 * Also generated from the character database. Whole-block approximations are wrong
 * in both directions — the Misc Symbols block is mostly narrow (U+2713 is one
 * column) while scattered code points inside it are wide.
 */
const WIDE = [
    [0x1100, 0x115f],
    [0x231a, 0x231b],
 ...
 */
export const RANGE_TABLES = { ZERO_WIDTH, WIDE };
...
export function sliceToWidth(text, columns) {
    let out = "";
    let width = 0;
    for (const char of text) {
...
function tokenize(text) {
    ...
    for (let i = 0; i < text.length;) {
        ...
        tokens.push({ ansi: false, text: char, width: charWidth(char.codePointAt(0)) });
        i += char.length;
    }
    return tokens;
}
 ...
 */
export function truncateStyled(text, columns, ellipsis = "\u2026") {
    if (columns <= 0)
        return "";
