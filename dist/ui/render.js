...
export function box(content, options) {
    ...
    const dim = (s) => `${theme.dim}${s}${theme.reset}`;
    const capLine = (label) => {
 ...
 */
function statusLine(state, pack, theme, width) {
    ...
    if (s.accuracy > 0)
        parts.push(`${Math.round(s.accuracy * 100)}% right`);
    while (parts.length > 1 && parts.join(DOT).length > width)
        parts.pop();
    return `${theme.dim}${parts.join(DOT)}${theme.reset}`;
}
function progressBar(fraction, width, theme) {
...
function moodFor(state) {
    ...
    if (state.mode === "caughtup")
        return "proud";
    if (state.mode === "waiting")
        return "asleep";
    return "watching";
}
export function renderFrame(state, pack, width, theme) {
    const inner = Math.max(10, width - 2);
    const pad = "  ";
    ...
    if (state.mode === "welcome") {
        ...
        if (!met.length) {
            ...
            for (const { word, item } of met.slice(from, from + perPage)) {
                ...
                push(`${theme.dim}#${String(word.rank).padEnd(4)}${theme.reset}` +
        ...
        switch (state.mode) {
            ...
            case "teach": {
                ...
                if (state.enrichPending)
                    push(`${theme.dim}asking Claude for a memory hook…${theme.reset}`);
            ...
            case "question": {
                ...
                if (card.kind === "recognize") {
                    ...
                    for (const line of wrap(card.prompt, body))
                        push(`${theme.bold}${line}${theme.reset}`);
                }
                else {
            ...
            case "feedback": {
                ...
                if (item) {
                    ...
                    push(`${theme.dim}box ${item.box}/${MAX_BOX}${DOT}back as "${nextKind}" in ${relative(item.due - state.now)}${theme.reset}`);
                }
                if (state.enrichPending)
                    push(`${theme.dim}asking Claude for a memory hook…${theme.reset}`);
                for (const line of state.enrichment ? wrap(state.enrichment, body) : []) {
    ...
    if (!state.showHelp && !isOnboarding(state)) {
        ...
        if (state.message)
            push(`${theme.dim}${state.message}${theme.reset}`);
    }
    ...
    const marker = state.agent === "busy" ? `${theme.green}●${theme.reset}` : `${theme.dim}○${theme.reset}`;
    ...
    if (state.mode === "welcome")
        ...
        footer = "p practise · t stats · w words · ? help · q quit";
        ...
        footer = "t stats · w words · l language · ? help · q quit";
