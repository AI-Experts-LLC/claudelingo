...
import { DEFAULT_MODEL, cachedHook, generatePack, hasClaude, memoryHook, } from "./enrich.js";
...
const VALUE_FLAGS = ["lang", "width", "source", "model", "count", "code", "choice", "text"];
function requireFlagValues(flags) {
    for (const name of VALUE_FLAGS) {
        if (flags[name] === "")
            fail(`--${name} needs a value`);
    }
}
function settingsFrom(flags) {
    requireFlagValues(flags);
    const loaded = loadSettings();
    const settings = { ...loaded.settings };
    if (typeof flags.lang === "string" && flags.lang)
        settings.lang = flags.lang;
    if (flags["always-on"] === true)
        settings.alwaysOn = true;
    if (flags["always-on"] === false)
        settings.alwaysOn = false;
    if (flags.enrich === false)
        settings.enrich = false;
    if (typeof flags.model === "string" && flags.model)
        settings.model = flags.model;
    if (flags["auto-pane"] === false)
        settings.autoPane = false;
    if (flags["auto-pane"] === true)
        settings.autoPane = true;
    if (flags.ask === true)
        settings.askFirst = true;
    if (flags.ask === false)
 ...
 */
function loadProgress(lang, mutate = true) {
    ...
    if (result.ok) {
        ...
        const wellFormed = stored?.version === 1 && typeof items === "object" && items !== null && !Array.isArray(items);
        if (wellFormed)
            return { progress: { ...emptyProgress(lang), ...stored } };
        return setAside(lang, file, "is not a version 1 deck", mutate);
    }
    if (result.reason === "missing")
        return { progress: emptyProgress(lang) };
    ...
    return {
        ...
        problem: `${file} could not be read (${detail}). Not saving, so your deck is left ` +
...
function setAside(lang, file, why, mutate) {
    if (!mutate) {
...
function cmdHook(args) {
    ...
    if (!event)
        fail("hook needs an event name, e.g. `claudelingo hook Stop`");
...
function cmdInit(args) {
    ...
    try {
        ...
        if (skill.state === "linked")
            process.stdout.write(`/lingo skill linked into ${skill.path}\n`);
        else if (skill.state === "already")
            process.stdout.write("/lingo skill already installed.\n");
        else if (skill.state === "plugin")
            process.stdout.write("/lingo comes from the plugin.\n");
            ...
            process.stderr.write(`/lingo NOT installed: ${skill.path} exists and is not ours; leaving it alone.\n`);
        }
    }
    catch (error) {
        // Not fatal: the pane and the status line work without it.
        process.stderr.write(`/lingo skill NOT installed: ${error.message}\n`);
    }
    try {
        ...
        failed.push("Codex");
        process.stderr.write(`Codex notify NOT installed: ${error.message}\n`);
    }
...
function cmdUninit(args) {
    ...
    try {
        const file = claudeCode.settingsPath(scope);
        ...
        failed.push(`Claude Code hooks: ${error.message}`);
    }
    try {
        if (claudeCode.uninstallSkill(fileURLToPath(import.meta.url)))
            removed.push("the /lingo skill");
    }
    catch (error) {
        failed.push(`/lingo skill: ${error.message}`);
    }
    try {
        ...
        failed.push(`Codex notify: ${error.message}`);
    }
    if (removed.length)
    ...
    if (failed.length) {
        for (const message of failed)
            process.stderr.write(`NOT removed — ${message}\n`);
        process.exit(1);
    }
}
/**
 ...
 */
function drillHook(pack, progress, lang, now) {
...
async function cmdStatusline(args) {
    try {
        ...
        const pending = value && typeof value.question === "string" && Array.isArray(value.choices)
            ? {
                ...
                choices: value.choices.filter((c) => typeof c === "string"),
        ...
        const agent = status && (status.state === "busy" || status.state === "idle")
...
async function cmdSessionStart(args) {
    try {
        ...
        const passEnv = {};
        if (process.env.CLAUDELINGO_HOME)
            passEnv.CLAUDELINGO_HOME = process.env.CLAUDELINGO_HOME;
...
async function cmdLaunch(args, agentArgs) {
    const { settings } = settingsFrom(args.flags);
    ...
    if (result.plan.kind === "none" || result.plan.reason) {
        process.stderr.write(`claudelingo: ${result.plan.reason ?? "could not open the pane"}. ` +
...
function cmdLangs() {
    ...
    for (const code of codes) {
        try {
            ...
            process.stdout.write(`${code.padEnd(6)}unreadable: ${error.message}\n`);
        }
    }
}
async function cmdPack(args) {
    if (args.rest[0] !== "generate")
        fail("usage: claudelingo pack generate --lang <name>");
    const language = args.flags.lang || args.rest[1];
    if (!language)
        fail("pack generate needs --lang, e.g. --lang Portuguese");
    const code = args.flags.code || language.slice(0, 2).toLowerCase();
    ...
    if (!args.flags.code && listPacks().includes(code)) {
        fail(`"${code}" is already in use (the default code is the first two letters of ` +
...
function cmdReset(args) {
    ...
    if (!held.ok) {
        if (held.reason === "held") {
            ...
            fail(`${who} is studying ${settings.lang} and would write its deck back over the ` +
...
function resolveWidth(raw) {
    ...
    if (!Number.isFinite(value) || value < MIN_WIDTH)
        return undefined;
...
async function cmdRun(args) {
    ...
    if (!acquired.ok && acquired.reason === "held") {
        ...
        fail(`${who} is already studying ${pack.englishName}. ` +
            `Close it, or run this one with a different --lang. ` +
            `If no other pane is running, delete ${lockFile}.`);
    }
    const problems = {};
    if (settingsProblem)
        problems.settings = settingsProblem;
    if (progressProblem)
        problems.deck = progressProblem;
    if (!acquired.ok) {
    ...
    if (!fs.existsSync(statusFile)) {
        try {
            ...
            problems.statusWrite = `cannot record agent state: ${error.message}`;
        }
    }
    const enrich = settings.enrich && hasClaude()
        ? (word) => memoryHook(word, pack, { model: settings.model })
        : undefined;
 ...
 */
function subcommandIndex(argv, name) {
    const takesValue = new Set(["lang", "width", "source", "model", "count", "code", "home", "choice", "text"]);
    for (let i = 0; i < argv.length; i++) {
        const arg = argv[i];
        ...
        if (!flag.includes("=") && takesValue.has(flag))
            i++;
    }
    return -1;
}
export async function main(argv = process.argv.slice(2)) {
    ...
    switch (args.command) {
        ...
        case "unknown":
            return fail(`unknown command "${args.rest[0] ?? ""}"\n` +
