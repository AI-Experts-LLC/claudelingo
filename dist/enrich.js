 ...
 */
export function generatePack(language, code, count, options = {}) {
    const { onProgress, existing, ...askOptions } = options;
    const asker = (prompt, { system }) => ask(prompt, { ...askOptions, system, timeoutMs: askOptions.timeoutMs ?? PACK_TIMEOUT_MS });
