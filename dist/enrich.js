...
export async function generatePack(language, code, count, options = {}) {
    const raw = await ask(`Produce the ${count} most common words in ${language} as JSON of the form ` +
        `{"code":"${code}","name":"<the language's own name>","englishName":"${language}",` +
        `"words":[{"term":"","gloss":"","pos":"","note":""}]}. Omit "note" when it does not apply.`, { ...options, system: PACK_SYSTEM, timeoutMs: options.timeoutMs ?? PACK_TIMEOUT_MS });
