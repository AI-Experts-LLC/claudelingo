 ...
 */
export function findNotify(toml, bin) {
    ...
    let depth = opens(lines[start]) - closes(lines[start]);
    while (depth > 0 && end + 1 < lines.length) {
        end += 1;
        depth += opens(lines[end]) - closes(lines[end]);
    }
 ...
 */
export function withNotify(toml, bin) {
    const found = findNotify(toml, bin);
    if (found?.ours)
        return toml;
    if (found) {
        throw new CodexConfigError(`${codexConfigPath()} already sets a different notify program on line ${found.start + 1}. ` +
...
export function removeNotify(toml, bin) {
    const found = findNotify(toml, bin);
    if (!found?.ours)
        return toml;
    const lines = toml.split("\n");
...
export function installNotify(bin, file = codexConfigPath()) {
...
export function uninstallNotify(bin, file = codexConfigPath()) {
 ...
 * does not recognise.
 */
export function classifyRolloutLine(line) {
    let entry;
    try {
...
export function classifyNotification(json) {
    try {
 ...
 */
export function watchCodexSession(onState, options = {}) {
    ...
    const consumed = new Map(seed.files.map((f) => [f.file, f.size]));
     ...
     */
    const watchlist = new Set(seed.files.slice(-RESUME_CANDIDATES).map((f) => f.file));
    ...
    const setCarry = (value) => {
        if (file)
            carries.set(file, value);
    };
    const recovered = () => {
        failures = 0;
        if (reported) {
    ...
    const poll = () => {
        ...
        if (due) {
            ...
            for (const seen of watchlist)
                scanner.refresh(seen);
    ...
    function readNewBytes(file) {
        ...
        try {
            ...
            fail(`cannot stat ${file}: ${error.message}`);
            return;
        }
        if (size < offset) {
