...
async function packChunk(ask, language, from, size, known) {
    ...
    const raw = await ask(`Produce words ranked ${from} to ${from + size - 1} by frequency in ${language}, ` +
        ...
        `again.${avoid} Omit "note" when it does not apply.`, { system: PACK_SYSTEM });
    const body = raw
        .replace(/^```(?:json)?\s*/i, "")
        .replace(/```\s*$/, "")
        .trim();
 ...
 */
export async function generatePack(ask, language, code, count, options = {}) {
    ...
    for (let chunk = 0; chunk < maxChunks && words.length < count && barren < 3; chunk++) {
        ...
        try {
            ...
            try {
                entries = await packChunk(ask, language, from, Math.max(20, Math.floor(size / 2)), [...seen].slice(-120));
            }
            catch (retryError) {
...
//# sourceMappingURL=packgen.js.map